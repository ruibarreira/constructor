...
class BuildIso(threading.Thread):
    ...
    def __init__(self, distroPath, queue):
        ...
        # Check for old dir
        oldDir = join(self.bootPath, "trail")
        if exists(oldDir):
    ...
    def get_webseeds(self):
        ...
        try:
            ...
            url = config.get("MIRRORSURL", "https://raw.githubusercontent.com/ruibarreira/constructor/master/mirrors.txt")
...
class DistroGeneral(object):
    def __init__(self, distroPath):
        ...
        self.description = "Debian"
        infoPath = join(self.rootPath, "etc/trail/info")
        if exists(infoPath):
