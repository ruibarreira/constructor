...
if [ -e /usr/bin/startxfce* ]; then
  sed_append_sting '^background\s*=.*' 'background=/usr/share/images/desktop-base/lightdmbg-flat.png' $CONF
  sed_append_sting '^theme-name\s*=.*' 'theme-name=greybird-trail' $CONF
elif [ -e /usr/bin/startkde* ]; then
  sed_append_sting '^background\s*=.*' 'background=/usr/share/images/desktop-base/lightdmbg-flat.png' $CONF
  sed_append_sting '^theme-name\s*=.*' 'theme-name=greybird-gtk3' $CONF
else
...
if [ -e $CONF ]; then
  ...
  sed -i -r 's/^#?(autologin-user)\s*=.*/\1=trail/' $CONF
...
ufw allow to any app CIFS
ufw allow from any app CIFS
ufw enable
